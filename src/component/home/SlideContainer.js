...
function Slide() {
  ...
  const filteredDataBlog = dataBlog.filter(
    (data) => data.id_cate === "01J61ZZFHK6ZY1W6GBY0JV2H4X"
  );
  return (
    <SliderContainer>
      <Carousel responsive={responsive}>
        {filteredDataBlog.map((data) => (
          <DivCard key={data.id}>
            <DivPic>
              <DivPicInside src={data.pic} alt={data.title} />
              {data.id_cate === "01J61ZZFHK6ZY1W6GBY0JV2H4X" && (
                <DivCatagory background={"/bookicon.png"} />
              )}
            </DivPic>
            <TitleCard>{data.title}</TitleCard>
            <Dcs dangerouslySetInnerHTML={{ __html: data.dcs }} />
            <DivBottom>
              <DivButton>
                <SeeMore
                  onClick={() =>
