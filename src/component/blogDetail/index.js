...
import TextDetail from "./DetailBlog";
...
function TextBlogList() {
  ...
  const [textDetail, setTextDetail] = useState(dataBlog[0]);
  useEffect(() => {
    setTextDetail(
      dataBlog.filter(
        (blog) => blog.id === window.location.pathname.split("/")[2]
      )[0]
    );
  }, []);
